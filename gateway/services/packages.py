...
(BEFORE_AFTER), `GGP22:` (CAREER_PATH), and `GGP23:` (RISK_IT).
MAP_THE_CAREER reuses `GGP6:` (SORTING_TIMELINE) since it is a variant of
that pre-existing taxonomy, not a new one.
...
PACKAGE_ID_RE = re.compile(r"^GGP([4-9]|1[0-9]|2[0-3])?:[0-9a-f]{24}$")
...
def _safe_filename_for_id(package_id: str) -> Path:
    ...
    character outside PACKAGE_ID_RE's fixed GGP(4-9|10-23)?:[0-9a-f]{24}
