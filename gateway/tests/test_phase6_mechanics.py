...
def test_ten_playable_cfb_ideas_stay_in_domain_and_never_pad():
    ...
    for c in result["concepts"]:
        ...
        assert c["playability_status"] == "PLAYABLE_NOW"
        assert c["preview"]["qa_status"] == "PASSED"
    # Universal Data Reuse pass: CFB now genuinely has a POSITION_LINEUP_GRID
    # capability shape -- the Rivalry Data + Gold Standard Content
    # Integration operation registered several real CFB puzzle capabilities
    # (CFB_FILL_THE_COLLEGES, CFB_ODD_COLLEGE_OUT, CFB_SPOT_THE_FAKE_LINEUP,
    # etc.) built on the real, curated 60-board SB_CHAMPION offense-by-
    # college dataset -- the exact "position lineup board" shape the
    # previous 6-mechanic ceiling comment said CFB had no equivalent of.
    # 7 distinct mechanics is CFB's real current maximum now. Same
    # discipline as before ("never hard-code a static list just to make a
    # test pass if Creator Intelligence already has a proper capability-
    # discovery mechanism") -- updated to match concepts.py's real, live,
    # capability-driven discovery rather than defending a ceiling that's no
    # longer real.
    mechanics = {c["core_mechanic"] for c in result["concepts"]}
