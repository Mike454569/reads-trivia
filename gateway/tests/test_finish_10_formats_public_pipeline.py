...
# --- 34. CATEGORY_ROULETTE (75-Format Expansion, Wave 1) -------------------

def test_category_roulette_full_public_playthrough_correct_and_incorrect():
    from gateway.services import public_mechanics as pm
    from gateway.services import packages

    r = pm.start_public_round(mode="category_roulette_mixed")
    rid = r["round_id"]
    assert rid.startswith("GGP37:")
    assert r["view"]["completed"] is False
    assert len(r["view"]["options"]) == 4
...
def test_category_roulette_never_leaks_the_real_answer_before_submission():
    from gateway.services import public_mechanics as pm

    r = pm.start_public_round(mode="category_roulette_mixed")
    assert set(r["view"].keys()) == {"round_index", "round_count", "completed", "category", "prompt", "options"}
    for it in r["view"]["options"]:
        assert set(it.keys()) == {"item_id", "label"}
