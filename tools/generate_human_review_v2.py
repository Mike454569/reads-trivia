#!/usr/bin/env python3
"""Human review package generator for Pilot v2. Read-only.

Reruns the exact same deterministic pipeline as
tools/export_quiz_engine_pilot_v2.py to recover the per-question audit
fields (draft year, raw team code, resolved franchise, source/domain) that
the pipeline computes but does not persist into data/quiz-engine-pilot-v2.js.
Before writing anything, this script loads the ACTUAL, already-written
data/quiz-engine-pilot-v2.js and asserts the reconstruction matches it
exactly, field by field, for all 100 questions. If there is any mismatch,
it aborts instead of writing a review that could misrepresent the real
output -- this is not a separate approximation of the pipeline, it is the
real output with its audit trail re-attached and verified.
"""
...
PILOT_V2_JS_PATH = REPO_ROOT / "data" / "quiz-engine-pilot-v2.js"
REVIEW_PATH = REPO_ROOT / "QUIZ_ENGINE_PILOT_V2_HUMAN_REVIEW.md"
...
def rebuild():
    c = gf.connect()
    spec = {
        "description": "which team drafted this nfl player", "competition_id": "NFL", "mechanic": "guess",
        "entity_type": "nfl_player", "relationship_predicate": "DRAFTED_BY", "object_type": "team",
...
def load_persisted_v2_js():
    text = PILOT_V2_JS_PATH.read_text(encoding="utf-8")
    m = re.search(r"window\.QUIZ_DATA_ENGINE_PILOT_V2 = (.*);\s*\Z", text, re.S)
    if not m:
        raise SystemExit("ABORT: could not parse data/quiz-engine-pilot-v2.js")
    ...
    for i, (r, p) in enumerate(zip(rebuilt, persisted)):
        ...
        if r_slim != p:
            raise SystemExit(
                ...
                f"persisted data/quiz-engine-pilot-v2.js.\nreconstructed={r_slim}\npersisted={p}"
            )
...
def write_review(rebuilt):
    ...
    lines = []
    lines.append("# Quiz Engine Pilot v2 -- Human Review Package")
    lines.append("")
    lines.append(
        "Regenerated from the same deterministic pipeline as "
        "`tools/export_quiz_engine_pilot_v2.py` (seed `" + SEED + "`) and verified "
        "byte-identical, field-by-field, against the already-written "
        "`data/quiz-engine-pilot-v2.js` before this document was produced -- this is "
    ...
    lines.append(f"- Team-code resolution: `team_aliases`, including the 30 SAFE_FIX_AVAILABLE corrections applied per `TEAM_ALIAS_SAFE_FIX_CHANGELOG.md`")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    for q in rebuilt:
        ...
        lines.append(f"- **Draft year:** {q['draft_season']}")
        lines.append(f"- **Raw historical team code:** `{q['draft_team_code']}`")
        lines.append(f"- **Resolved franchise:** `{q['franchise_id']}` (\"{q['options'][q['correctIndex']]}\"), resolved via Engine's `team_aliases` table, season-matched to {q['draft_season']}")
        lines.append(f"- **Source/domain:** `draft_facts` row, player_key `{q['player_key']}`, verification_status `{q['verification_status']}`, source_id `{q['source_id']}` "
                      f"(domain `NFL_DRAFT`); relationship generated via Game Factory predicate `DRAFTED_BY` (source table reported by Engine: `{q['source_table']}`).")
        lines.append("")
...
def main():
    rebuilt = rebuild()
    persisted = load_persisted_v2_js()
    verify_match(rebuilt, persisted)
    write_review(rebuilt)
    print(f"Verified {len(rebuilt)} questions match data/quiz-engine-pilot-v2.js exactly.")
