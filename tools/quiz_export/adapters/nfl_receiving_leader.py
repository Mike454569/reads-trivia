"""NFL game receiving leader -- objective "top offensive performer"
(receiving category). See _nfl_game_leader_common.py for the real, shared
logic.
"""
...
OUT_PATH = None
CATEGORY = "NFL Game Receiving Leader"
TRACK_ENTITY = True
STAT_COLUMN = "rec_yards"
...
def evaluate(c, row, rng, guard):
    return common.evaluate(c, row, rng, guard, stat_column=STAT_COLUMN, stat_label=STAT_LABEL, category=CATEGORY, entity_prefix="nfl_rec_ldr")
...
def header_lines(seed: str) -> list[str]:
    return ["// Director-pipeline-only domain.", "// tools/quiz_export/adapters/nfl_receiving_leader.py.", f"// Deterministic seed: \"{seed}\"."]
