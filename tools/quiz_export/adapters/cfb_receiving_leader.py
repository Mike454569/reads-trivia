"""CFB game receiving leader -- objective "top offensive performer"
(receiving category). See _game_leader_common.py for the real, shared
logic.
"""
...
OUT_PATH = None
CATEGORY = "CFB Game Receiving Leader"
TRACK_ENTITY = True
...
def evaluate(c, row, rng, guard):
    return common.evaluate(
        ...
        entity_prefix="cfb_rec_ldr", min_season=MIN_SEASON, max_season=MAX_SEASON,
    )
...
def header_lines(seed: str) -> list[str]:
    return ["// Director-pipeline-only domain.", "// tools/quiz_export/adapters/cfb_receiving_leader.py.", f"// Deterministic seed: \"{seed}\"."]
