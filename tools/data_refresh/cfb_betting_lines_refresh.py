...
def run_cfb_betting_lines_refresh(seasons: list[int] | None = None) -> dict:
    ...
    # current in-progress season looks like a catastrophic drop against
    # 20+ untouched prior seasons.
    ...
    baseline_count = c.execute(
        f"SELECT COUNT(*) FROM cfb_betting_lines WHERE season IN ({','.join('?' * len(target_seasons))})",
        target_seasons,
    ).fetchone()[0]
